...
Pipeline:
  [check_new_sessions]         @task.short_circuit (Gatekeeper)
      ↓
  [extract_api_to_s3]          @task (TaskFlow)
      ↓
...
S3 Partition Layout:
  s3://{S3_BUCKET}/openf1/{endpoint}/meeting_key={MK}/session_key={SK}/data.json
...
def _build_jinja_sql_template(endpoint: str, cfg: dict) -> str:
    ...
    {{% if sessions %}}
        {{% for session in sessions %}}
            ...
            FILE_FORMAT = (TYPE = 'JSON' STRIP_OUTER_ARRAY = TRUE NULL_IF = ('None', 'null', 'NULL', ''));
...
def openf1_session_pipeline() -> None:
    ...
    def check_new_sessions(data_interval_start=None, data_interval_end=None) -> list[dict] | bool:
        ...
        return active_sessions # Data ini akan otomatis masuk ke XCom

    # ── Task 2 : Extract ─────────────────────────────────────────────────────
    @task
    def extract_api_to_s3(sessions_to_load: list[dict]) -> None:
        """
        Menarik data API *hanya* untuk session_key yang dikirimkan oleh task pertama.
        """
        ...
        for session in sessions_to_load:
            ...
            for endpoint in ENDPOINTS.keys():
                # Endpoint meetings dipanggil berdasarkan meeting_key
                ...
                s3_key = f"{S3_ROOT}/{endpoint}/meeting_key={mk}/session_key={sk}/data.json"
                s3.load_string(
                ...
                log.info("[%s] Sesi %s tersimpan di s3://%s/%s", endpoint, sk, S3_BUCKET, s3_key)
    ...
    # ── Wiring ───────────────────────────────────────────────────────────────
    # TaskFlow API akan mengalirkan output list[dict] dari check_new_sessions
    # langsung masuk ke dalam parameter extract_api_to_s3
    active_sessions = check_new_sessions()
    
    extract_task = extract_api_to_s3(active_sessions)
    
    # Atur urutan dependensinya
    extract_task >> load >> dbt_staging >> dbt_intermediate >> dbt_marts

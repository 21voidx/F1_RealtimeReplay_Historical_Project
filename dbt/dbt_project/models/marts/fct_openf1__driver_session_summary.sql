...
driver_session_summary as (
    select
        ...
        coalesce(p.pit_event_count, 0) as pit_event_count,
        coalesce(p.pit_stop_count, 0) as pit_stop_count,
        p.avg_pit_duration_seconds,
        p.fastest_pit_duration_seconds,
        p.slowest_pit_duration_seconds,
        p.avg_pit_lane_duration_seconds,
        p.fastest_pit_lane_duration_seconds,
        p.slowest_pit_lane_duration_seconds,
        p.avg_raw_pit_duration_seconds,
        p.pit_laps,
        p.first_pit_at,
        p.last_pit_at,
